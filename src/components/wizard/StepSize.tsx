...
import { ArrowRight, ArrowLeft, RectangleHorizontal, RectangleVertical, Sparkles, Shield, Gem, Check, RotateCw, ZoomIn, ZoomOut, Move, Package, Percent, ImagePlus, Upload, X, Image, Layers } from "lucide-react";
...
type TabMode = "single" | "collection";
...
const StepSize = ({ imageUrl, sizeIdx, material, onSelect, onSelectMaterial, onNext, onBack, onSelectBundle, selectedBundle, bundleSlots, onUpdateSlot, onClearBundle }: Props) => {
  ...
  const [tab, setTab] = useState<TabMode>(selectedBundle ? "collection" : "single");
  ...
  const handleTabSwitch = (newTab: TabMode) => {
    setTab(newTab);
    if (newTab === "single" && isBundle) {
      onClearBundle();
    }
  };

  const isMetal = material.startsWith("metal");

  return (
    <div className="space-y-4">
      <div className="text-center">
        <h2 className="text-3xl md:text-4xl font-display font-bold text-foreground">
          {isBundle ? selectedBundle!.name : "Size & Material"}
        </h2>
        <p className="text-muted-foreground font-body mt-1 tracking-wide text-sm">
          {isBundle
            ...
            : "Select your perfect size, medium, and style."}
        </p>
      </div>

      {/* Tab switcher — Single Print vs Curated Collections */}
      <div className="flex justify-center">
        <div className="inline-flex bg-secondary rounded-full p-0.5 gap-0.5">
          <button
            onClick={() => handleTabSwitch("single")}
            className={`flex items-center gap-2 px-5 py-2 rounded-full text-xs font-body font-semibold tracking-wider uppercase transition-all ${
              tab === "single"
      ...
      {tab === "collection" && !isBundle && (
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            {bundles.map((bundle) => (
              <Card
                key={bundle.id}
                className="overflow-hidden cursor-pointer border-border hover:border-primary/40 hover:bg-primary/5 transition-all group"
                ...
                <div className="p-4 space-y-2">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-sm font-display font-bold text-foreground">{bundle.name}</h4>
                    </div>
                    {bundle.tag && (
                      <Badge className="bg-gradient-gold text-primary-foreground border-0 text-[8px] font-body uppercase tracking-wider">
                        {bundle.tag}
                      </Badge>
                    )}
                  </div>
                  <p className="text-[11px] text-muted-foreground font-body leading-relaxed">{bundle.description}</p>
                  <div className="flex items-center gap-2 pt-1">
                    <span className="text-lg font-display font-bold text-gradient-gold">${bundle.salePrice}</span>
                    <span className="text-xs text-muted-foreground font-body line-through">${bundle.originalPrice}</span>
      ...
      {/* ===== BUNDLE SELECTED — Image slots + preview ===== */}
      {tab === "collection" && isBundle && selectedBundle && (
        <>
          {/* Bundle wall preview */}
          <div className="flex justify-center">
            {(() => {
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-lg p-3 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => { onClearBundle(); setTab("collection"); }} className="font-body text-[10px] h-7 gap-1 tracking-wider">
                <ArrowLeft className="w-3 h-3" /> Change Collection
              </Button>
            </div>
            ...
            return (
              ...
              >
                <div className="aspect-[16/9] relative overflow-hidden">
                  ...
                  {mat.id.startsWith("metal") && !isBundle && (() => {
                ...
                <div className="p-2 text-center">
                  ...
                  {!isBundle && <p className="text-sm font-display font-bold text-gradient-gold mt-0.5">${price}</p>}
                </div>
              </Card>
            );
          })}
      ...
      <div className="flex justify-between pt-4">
        ...
        {isBundle ? (
          ...
          >
            {isMetal ? "Personalize" : "Finishing"} <ArrowRight className="w-4 h-4" />
          </Button>
        ) : (
